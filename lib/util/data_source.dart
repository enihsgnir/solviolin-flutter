...
Future<void> funcFirst() async {
  Client client = Get.put(Client());
  DataController _controller = Get.find<DataController>();

  final today1 = DateTime.now();
  final first1 = DateTime(today1.year, today1.month, 1);
  final last1 = DateTime(today1.year, today1.month + 1, 0);
  ...
  _controller.updateMyValidReservations(await client.getReservations(
    branchName: "잠실",
    startDate: first1,
    endDate: last1.add(Duration(hours: 23, minutes: 59, seconds: 59)),
    userID: "sleep",
    bookingStatus: [-3, -1, 0, 1, 3],
  )
    ..sort((a, b) => a.startDate.compareTo(b.startDate)));

  _controller.updateAvailables(await client.getAvailable(
    branchName: "잠실",
    teacherID: _controller.teachers[0].teacherID,
    startDate: today1,
  )
